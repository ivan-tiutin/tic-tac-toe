...
export default function App() {
    ...
    const resetGame = (isNewRound: boolean): void => {
        setState((prev) => {
            const stateClone = structuredClone(prev);
    ...
    return (
        <>
            <main>
                <div className="grid">
                    <div className={classNames("turn", game.currentPlayer.colorClass)} data-id="turn">
            ...
            {game.status.isComplete && (
                <Modal
                    message={game.status.winner ? `${game.status.winner.name} wins!` : "Tie!"}
